...
$(document).ready(function() {
    // Assuming that the ZeroClipboard swf file is in the same folder than big002,
    // get the path to it (it will be relative to the current page location).
    var bbScriptPath = $('script[src$="big002.js"]').attr('src');  // the js file path
    var bbPathToZeroClipboardSwf = bbScriptPath.replace('big002.js', 'ZeroClipboard.swf');
    ...
    $(document).on('mouseenter', '.org-src-container', function () {
        ...
        if ((window.location.protocol != 'file:') && ($(this).find('.zclip').length == 0)) {
            $(this).find('.copy-to-clipboard-button').zclip({
                ...
                //path: 'Styles/big002/js/ZeroClipboard.swf',
